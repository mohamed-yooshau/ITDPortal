...
export async function getAuthConfig() {
  ...
  if (res.status === 426) {
    ...
    const retryData = (await retry.json()) as { encConfig?: EncBundle; localLoginEnabled?: boolean; authEncryptionEnabled?: boolean };
    if (retryData.encConfig) {
      const decrypted = await decryptPayload(retryData.encConfig, "auth/config");
      return decrypted as { localLoginEnabled: boolean; authEncryptionEnabled: boolean };
    }
    return {
  ...
  const data = (await res.json()) as { encConfig?: EncBundle; localLoginEnabled?: boolean; authEncryptionEnabled?: boolean };
  if (data.encConfig) {
